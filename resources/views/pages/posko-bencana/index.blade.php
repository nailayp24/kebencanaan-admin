        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white border-bottom py-3">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 fw-semibold">
                        <i class="mdi mdi-home-group me-2 text-primary"></i>Data Posko Bencana
                    </h5>
                    <a href="{{ route('posko-bencana.create') }}" class="btn btn-primary btn-sm">
            ...
            <div class="card-body p-4">
                ...
                <div class="filter-container bg-light-subtle rounded-3 p-4 mb-4 border">
                    <form method="GET" action="{{ route('posko-bencana.index') }}">
                        <div class="row g-3 align-items-end">
                                    ...
                                    <select name="kejadian_id" class="form-select form-select-sm" onchange="this.form.submit()">
                                        ...
                                        @foreach($kejadianOptions as $kejadian)
                                            <option value="{{ $kejadian->kejadian_id }}"
                                                ...
                                                {{ $kejadian->jenis_bencana }} - {{ Str::limit($kejadian->lokasi_text, 40) }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="search-box">
                                    <div class="input-group input-group-sm">
                                        ...
                                        <input type="text" name="search" class="form-control border-start-0"
                                               ...
                                               placeholder="Cari nama posko, alamat, kontak...">
                                        @if(request('search'))
                            ...
                            {{-- Reset Button --}}
                            <div class="col-md-1">
                                <a href="{{ route('posko-bencana.index') }}" class="btn btn-outline-secondary btn-sm w-100 d-flex align-items-center justify-content-center">
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            @forelse($posko as $item)
                                <tr>
                                    <td class="text-center text-muted">{{ ($posko->currentPage() - 1) * $posko->perPage() + $loop->iteration }}</td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="fw-medium">{{ $item->penanggung_jawab }}</div>
                                    </td>
                                    <td>
                                        <div class="d-flex gap-1 justify-content-center">
                                            ...
                                            <form action="{{ route('posko-bencana.destroy', $item->posko_id) }}"
                                                  method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-outline-danger px-2"
                                                        onclick="return confirm('Hapus posko {{ $item->nama }}?')"
                                                        title="Hapus">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-5">
                                        <i class="mdi mdi-home-remove-outline display-4 text-muted"></i>
                                        <div class="mt-3 text-muted">
                                            @if(request('kejadian_id') || request('search'))
                                                Tidak ada data posko yang sesuai dengan filter
                                            @else
                                                Belum ada data posko bencana
                                            @endif
                                        </div>
                                        @if(request('kejadian_id') || request('search'))
                                            <a href="{{ route('posko-bencana.index') }}" class="btn btn-outline-primary mt-2">
                                                <i class="mdi mdi-refresh me-1"></i> Reset Filter
                                            </a>
                                        @else
                                            <a href="{{ route('posko-bencana.create') }}" class="btn btn-primary mt-2">
                                                <i class="mdi mdi-plus me-1"></i> Tambah Posko Pertama
                                            </a>
                                        @endif
                                    </td>
                                </tr>
                ...
                @if($posko->hasPages())
                    <div class="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
                        <div class="text-muted small">
                            Menampilkan {{ $posko->firstItem() }} - {{ $posko->lastItem() }} dari {{ $posko->total() }} data
                        </div>
                        <div>
