{{-- resources/views/admin/donasi-bencana/index.blade.php --}}
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white border-bottom py-3">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 fw-semibold">
                        <i class="mdi mdi-hand-heart me-2 text-primary"></i>Data Donasi Bencana
                    </h5>
                    <a href="{{ route('donasi-bencana.create') }}" class="btn btn-primary btn-sm">
            ...
            <div class="card-body p-4">
                ...
                <div class="filter-container bg-light-subtle rounded-3 p-4 mb-4 border">
                    <form method="GET" action="{{ route('donasi-bencana.index') }}">
                        <div class="row g-3 align-items-end">
                            ...
                            <div class="col-md-3">
                                <label class="form-label fw-medium mb-2">
                                    <i class="mdi mdi-gift me-1 text-success"></i>Jenis Donasi
                                </label>
                                <div class="dropdown-filter">
                            ...
                            {{-- Reset Button --}}
                            <div class="col-md-1">
                                <a href="{{ route('donasi-bencana.index') }}" class="btn btn-outline-secondary btn-sm w-100 d-flex align-items-center justify-content-center">
                                    ...
                                    <div class="d-flex align-items-center flex-wrap gap-2">
                                        ...
                                        @if(request('jenis'))
                                            <span class="badge bg-success-subtle text-success border border-success-subtle d-flex align-items-center">
                                                ...
                                                {{ $jenisOptions[request('jenis')] ?? request('jenis') }}
                    ...
                    <table class="table table-hover mb-0">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th width="120" class="text-end">Nilai</th>
                        ...
                        <tbody>
                            @forelse($donasi as $item)
                                <tr>
                                    <td class="text-center text-muted">{{ ($donasi->currentPage() - 1) * $donasi->perPage() + $loop->iteration }}</td>
                                    <td>
                                        <div class="fw-medium">{{ $item->donatur_nama }}</div>
                                        @if ($item->keterangan)
                                    ...
                                    <td>
                                        @php
                                            $jenisColors = [
                                                'uang' => ['bg' => 'success-subtle', 'text' => 'success', 'icon' => 'cash'],
                                                'barang' => ['bg' => 'warning-subtle', 'text' => 'warning', 'icon' => 'package-variant'],
                                                'jasa' => ['bg' => 'info-subtle', 'text' => 'info', 'icon' => 'tools'],
                                            ];
                                            $jenisConfig = $jenisColors[$item->jenis] ?? ['bg' => 'secondary-subtle', 'text' => 'secondary', 'icon' => 'gift'];
                                        @endphp
                                        <span class="badge {{ $jenisConfig['bg'] }} text-{{ $jenisConfig['text'] }} border border-{{ $jenisConfig['text'] }}-subtle">
                                            <i class="mdi mdi-{{ $jenisConfig['icon'] }} me-1"></i>
                                            {{ $item->jenis_donasi ?? ucfirst($item->jenis) }}
                                        </span>
                                    </td>
                                    <td class="text-end">
                                        @if ($item->nilai)
                                            <div class="fw-bold">{{ $item->nilai_formatted }}</div>
                                        @else
                                    ...
                                    <td>
                                        <div class="d-flex gap-1 justify-content-center">
                                            <a href="{{ route('donasi-bencana.show', $item->donasi_id) }}"
                                            ...
                                            <form action="{{ route('donasi-bencana.destroy', $item->donasi_id) }}"
                                                ...
                                                <button type="submit" class="btn btn-sm btn-outline-danger px-2"
                                                        onclick="return confirm('Hapus donasi dari {{ $item->donatur_nama }}?')"
                                                        title="Hapus">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-5">
                                        <i class="mdi mdi-hand-heart-outline display-4 text-muted"></i>
                                        <div class="mt-3 text-muted">
                                            @if(request('kejadian_id') || request('jenis') || request('search'))
                                                Tidak ada data donasi yang sesuai dengan filter
                                            @else
                                                Belum ada data donasi bencana
                                            @endif
                                        </div>
                                        @if(request('kejadian_id') || request('jenis') || request('search'))
                                            <a href="{{ route('donasi-bencana.index') }}" class="btn btn-outline-primary mt-2">
                                                <i class="mdi mdi-refresh me-1"></i> Reset Filter
                                            </a>
                                        @else
                                            <a href="{{ route('donasi-bencana.create') }}" class="btn btn-primary mt-2">
                                                <i class="mdi mdi-plus me-1"></i> Tambah Donasi Pertama
                                            </a>
                                        @endif
                                    </td>
                                </tr>
